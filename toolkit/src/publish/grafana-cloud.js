...
export async function publishToGrafanaCloud(results, config) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.ok) {
        log.info({ status: response.status, target: results.target }, 'published to Grafana Cloud');
        return;
      }

      if (response.status === 429 && attempt < maxRetries) {
        const retryAfter = parseInt(response.headers.get('retry-after') || '5', 10);
        log.warn({ retryAfter, attempt }, 'rate limited, retrying');
      ...
      if (attempt < maxRetries) {
        log.warn({ error: err.message, attempt }, 'publish failed, retrying');
